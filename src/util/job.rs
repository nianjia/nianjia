...
//!
//! Most of the time when you're running nianjia you expect Ctrl-C to actually
//! terminate the entire tree of processes in play, not just the one at the top
//! (nianjia). This currently works "by default" on Unix platforms because Ctrl-C
...
//! not happen and Ctrl-C just kills nianjia.
//!
...
mod imp {
    ...
    pub unsafe fn setup() -> Option<()> {
        ...
        // when-nianjia-is-killed-subprocesses-are-also-killed, but that requires
        // one nianjia spawned to become its own session leader, so we do that
        // here.
        if env::var("__NIANJIA_TEST_SETSID_PLEASE_DONT_USE_ELSEWHERE").is_ok() {
            libc::setsid();
        }
        Some(())
    }
...
mod imp {
    ...
    pub unsafe fn setup() -> Option<Setup> {
        ...
        // to force Nianjia to abort in this situation or force others to *not*
