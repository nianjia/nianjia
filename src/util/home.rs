/// Canonical definitions of `home_dir`, `nianjia_home`, and `rustup_home`.
///
/// This provides the definition of `home_dir` used by Nianjia and
/// rustup, as well functions to find the correct value of
/// `NIANJIA_HOME` and `RUSTUP_HOME`.
///
...
/// Unix emulation environment like Cygwin or MinGW. Neither Nianjia nor
/// rustup use the standard libraries definition - they use the
/// definition here.
///
/// This crate further provides two functions, `nianjia_home` and
/// `rustup_home`, which are the canonical way to determine the
/// location that Nianjia and rustup store their data.
///
...
/// `.nianjia` or `NIANJIA_HOME`.
///
/// It returns one of the following values, in this order of
/// preference:
///
/// - The value of the `NIANJIA_HOME` environment variable, if it is
///   an absolute path.
/// - The value of the current working directory joined with the value
///   of the `NIANJIA_HOME` environment variable, if `NIANJIA_HOME` is a
///   relative directory.
/// - The `.nianjia` directory in the user's home directory, as reported
...
pub fn nianjia_home() -> io::Result<PathBuf> {
    let cwd = env::current_dir()?;
    nianjia_home_with_cwd(&cwd)
}

pub fn nianjia_home_with_cwd(cwd: &Path) -> io::Result<PathBuf> {
    let env_var = env::var_os("NIANJIA_HOME");

    // NB: During the multirust-rs -> rustup transition the install
    // dir changed from ~/.multirust/bin to ~/.nianjia/bin. Because
    // multirust used to explicitly set NIANJIA_HOME it's possible to
    // get here when e.g. installing under `nianjia run` and decide to
    ...
    let env_var = if let Some(v) = env_var {
       ...
       if vv.contains(".multirust/nianjia") ||
            vv.contains(r".multirust\nianjia") ||
            vv.trim().is_empty() {
           None
       } else {
           Some(v)
    ...
    let env_nianjia_home = env_var.map(|home| cwd.join(home));
    let home_dir = home_dir()
        .ok_or(io::Error::new(io::ErrorKind::Other, "couldn't find home dir"));
    let user_home = home_dir.map(|p| p.join(".nianjia"));

    // Compatibility with old nianjia that used the std definition of home_dir
    let compat_home_dir = dirs::home_dir();
    let compat_user_home = compat_home_dir.map(|p| p.join(".nianjia"));
    
    if let Some(p) = env_nianjia_home {
        Ok(p)
    } else {
